...
object CharactersApi:
  ...
  def addCharacter(name: String, nicknames: List[String], origin: Origin, role: Option[RoleArg]):  ZIO[CharactersApi, PrimaryError, CharacterId] =
   ZIO.serviceWithZIO(_.addCharacter(name, nicknames, origin, role))

  def updateCharacter(id: CharacterId, name: String, nicknames: List[String], origin: Origin, role: Option[RoleArg]):  ZIO[CharactersApi, PrimaryError, Boolean] =
